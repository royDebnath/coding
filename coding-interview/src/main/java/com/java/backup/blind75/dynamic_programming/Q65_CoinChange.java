package com.java.backup.blind75.dynamic_programming;
 ...
 * so coins required = dp[6] + 1 (here 1 coin for the current coin of value 1)
 *                   = 2+1 = 3
 *
 * for coin 3 :
 * remaining amount is 7-3 = 4. we have calculated dp[6].
 * so coins required = dp[4] + 1 (here 1 coin for the current coin of value 3)
 *                   = 1+1 = 2
 *
 * for coin 4 :
 * remaining amount is 7-4 = 3. we have calculated dp[3].
 * so coins required = dp[3] + 1 (here 1 coin for the current coin of value 4)
 *                   = 1+1 = 2
 *
 * for coin 5 :
 * remaining amount is 7-5 = 2. we have calculated dp[6].
 * so coins required = dp[2] + 1 (here 1 coin for the current coin of value 5)
 *                   = 2+1 = 3
 *
 *
    ...
    public static int coinChange(int[] coins, int targetAmount) {
        ...
        for(int amount=1; amount<=targetAmount; amount++){
            for(int j=0; j<coins.length; j++){
                ...
                if(deficit >=0 && // trying to fill the deficit with each coin
                        dp[deficit] != Integer.MAX_VALUE){ //this check makes sure, there is a possible way to fill the deficit
                                                          // if dp[deficit] has remained Integer.Max this means it is not possible to fill the deficit
                    dp[amount] = Math.min(dp[amount], dp[deficit]+1);
                }
            }
        }
