 ...
 *
 * Problem Description
 * Given a binary tree and a particular node p in it,
 * you need to find the "in-order successor" of that node.
 * The in-order successor for a given node p in a BST is defined as
 * the node with the smallest key that is greater than p.val.
 * If such a node doesn't exist, in other words, if p is the node with the highest value in the tree,
 * the function should return null.
 *
 * The in-order traversal of a BST produces the node values in an ascending order.
 * So the task could also be seen as finding the next node in the in-order traversal sequence.
 *
 * Example Walkthrough
 * Let's illustrate the solution approach using a simple binary search tree and
 * following the steps to find the in-order successor of a given node p. Consider the following BST:
 *
 *       4
 *      / \
 *     2   6
 ...
 *
 * Since 4 is greater than 3, the node with 4 could be the in-order successor.
 * We save this node and move to the left to look for an even closer value greater than 3.
 * Now, root is at the node with the value 2.
 * We compare p.val with the new root.val (2).
 * Since 2 is less than 3, we discard the left subtree including the current node and
 * move right to find a larger value.
