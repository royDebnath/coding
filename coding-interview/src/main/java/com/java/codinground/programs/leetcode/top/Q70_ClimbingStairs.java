 ...
 * This means that for each step we take, we have two choices - to take a single step or a double step -
 * unless we are at a point where only one step is needed to reach the top.
 *
 ...
 *
 * We translate this recursive relationship into an iterative solution by using two variables,
 * a and b,
 * to store the number of ways to reach the current and the next step.
 * We then use a simple loop to iterate through the steps, updating a and b at each iteration.
 * At each step in our loop, a takes the value of b (the number of ways to get to the previous step),
 * and b becomes a+b (the number of ways to get to the current step from the two steps before it).
 * By the time we reach the top of the staircase, b will hold the total number of distinct ways to
 * reach the top.
 *
 * Solution Approach
 *
 * First, we initialize two variables, a and b, to 0 and 1, respectively.
 * These variables will help us keep track of the number of ways to reach the current step
 * and the next step. In dynamic programming terms, a represents the solution to the
 * subproblem at i-1 and b represents the solution to the subproblem at i.
 *
 * We then use a for loop to iterate exactly n times, where n is the total number of steps in the staircase.
 * In each iteration, we perform the following actions:
 *
 ...
 *
 * Example Walkthrough
 * Let's use an example where n = 4, which represents a staircase with 4 steps to
 * illustrate the solution approach.
 *
 ...
 * Take 2 steps once.
 * For the third step (n = 3), we need to sum the ways to reach n = 1 and n = 2
 * because we can get to the third step by taking:
 *
 * 1 step from the second step (which has 2 ways as calculated before) or
 * 2 steps from the first step (which has 1 way as calculated before).
 * So the total number of ways to reach n = 3 is 1 (from n=1) + 2 (from n=2) = 3 ways.
 *
