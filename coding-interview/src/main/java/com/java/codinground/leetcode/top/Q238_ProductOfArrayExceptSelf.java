...
public class Q238_ProductOfArrayExceptSelf {
    ...
    public static int[] productExceptSelf(int[] nums) {
        ...
        int leftProductAtIndex = 1;
        for (int i = 0; i < length; i++) {
            result[i] = leftProductAtIndex * result[i];
            leftProductAtIndex = leftProductAtIndex * nums[i]; // set productAtIndex for next iteration
        }

        int rightProductAtIndex = 1;
        for (int i = length - 1; i >= 0; i--) {
            result[i] = rightProductAtIndex * result[i];
            rightProductAtIndex = rightProductAtIndex * nums[i];
        }
        return result;
    }
